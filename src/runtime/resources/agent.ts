...
import { toolingToToolGuide } from "../tool.ts"
...
import { resolveToolingGuides } from "./tooling.ts"
 ...
 * InstructionTemplate happens once at session instantiation.
 *
...
/** Runtime-resolved artifacts derived from the spec at instantiation. */
export interface AgentRuntime {
    ...
    * the tooling contributed by `extends` presets (presets prepended at
    * instantiation). Supports `toolset`, `route`, and `subagent` entries.
    */
...
export class AgentObject implements ResourceObject {
    ...
    * posture. Returns guides resolved against the session's resources
    * (toolset/skill/subagent) AND local `type: route` entries — the latter are
    * always reachable, even from no posture (entry / back-to-root).
    */
   getTools(): ToolGuide[] {
      return [
         ...resolveToolingGuides(this.runtime.tooling, this.session),
   ...
   static async fromManifest(
      ...
      const persona = resolveInstructionRef(
         spec.instruction,
