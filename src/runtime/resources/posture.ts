...
import { resolveToolingGuides, parseToolsetPattern, dedupGuardrails } from "./tooling.ts"
...
export class PostureObject implements ResourceObject {
   ...
   getActiveTooling(): ToolGuide[] {
       ...
       // entries (toolset/skill/subagent) AND local `type: route`
   ...
   static async fromManifest(
      ...
      const template = resolveInstructionRef(
         spec.instruction,
   ...
   resolveSkillTemplate(name: string): InstructionTemplate | undefined {
      for (const t of this.runtime.tooling) {
         ...
         for (const p of patterns) {
            ...
            const instructions = this.session.blueprint.instructions
            if (!instructions.has(resourceName)) continue
            const tmpl = instructions.get(resourceName)
   ...
   async activateSkill(
      ...
      if (!template) {
         context.deliver(deliveryId, { error: `Skill not found: ${name}` }, true)
         return undefined
      }
