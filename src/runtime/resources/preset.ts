 ...
 * instantiation by other objects (agent/posture/skill) via `spec.extends`. A
 * preset has NO runtime footprint of its own — it never exposes tools, never
 * activates, and never emits fragments. It only mutualises configuration. See
 * docs/resources.md.
 */
...
export class PresetObject implements ResourceObject, PresetView {
   ...
   /** Typed discriminator — a preset exposes itself as a `PresetView`. */
   asPreset(): PresetView {
      return this
   }
