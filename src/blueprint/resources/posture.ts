...
// Import the apiVersion constant directly from its origin (not from agent.ts)
// to avoid a posture ↔ agent import cycle that surfaces as a TDZ at module
// evaluation. agent.ts re-exports the same constant for back-compat.
 ...
 * by the caller). There is no virtual/builtin resource: every resource must
 * be declared in the blueprint. Tool-name filtering matches either the full
 * name or the suffix after `${resource}__`.
 */
